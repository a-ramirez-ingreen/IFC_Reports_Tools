...
if "df_filtrado" in st.session_state:
    ...
    with st.expander(" Seleccionar materiales a analizar"):
        materiales = df['Material'].dropna().unique().tolist()
        all_selected_key = "select_all_materials_unico"
        all_selected = st.checkbox("Seleccionar todos los materiales", value=True, key=all_selected_key)
        seleccionados = materiales if all_selected else []
        cols = st.columns(4)
        for i, material in enumerate(materiales):
            key_material = f"material_{material}"
            if all_selected or cols[i % 4].checkbox(material, key=key_material):
                if material not in seleccionados:
                    seleccionados.append(material)

    st.markdown("###  Selección de etapas del ciclo de vida")
    etapas_disponibles = ["A1-3", "A4", "A5", "C1", "C2", "C3", "C4", "D"]
    seleccionar_todo_key = "select_all_etapas_unico"
    seleccionar_todo = st.checkbox("Seleccionar todas las etapas", value=True, key=seleccionar_todo_key)
    etapas_seleccionadas = []
    cols_etapas = st.columns(len(etapas_disponibles))
    for i, etapa in enumerate(etapas_disponibles):
        key_etapa = f"etapa_{etapa}_unico"
        if seleccionar_todo or cols_etapas[i].checkbox(etapa, key=key_etapa, value=False):
            etapas_seleccionadas.append(etapa)
...
3. Detectar la unidad a partir del encabezado.
...
⚠️ Muy importante:
- La tabla debe tener solo una línea de encabezados.
...
"""
        ...
        with st.spinner(" Normalizando tabla para exportación IFC..."):
            ...
            try:
                ...
                # Mostrar solo la tabla final normalizada
                st.markdown("### 🧾 Resultado: Huella de carbono por elemento")
